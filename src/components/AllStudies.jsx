    ...
    <div style={{ padding: "24px 0" }}>
      ...
      <div style={{ background: C.surface, borderRadius: 8, border: `1px solid ${ C.border }`, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          ...
          <tbody>
            {filtered.map((s, i) => {
              const pct = Math.round((s.filled / (s.filled + s.missing + s.confirm)) * 100) || 0; return (
                <tr key={i} onClick={() => onSelect(s)} style={{ borderBottom: `1px solid ${ C.borderLight }`, cursor: "pointer" }} onMouseEnter={e => e.currentTarget.style.background = C.surfaceAlt} onMouseLeave={e => e.currentTarget.style.background = ""}>
                  ...
                  <td style={{ padding: "12px 14px" }}>
                    <button
                      onClick={(e) => {
                        ...
                        const id = s.id || s.studyId;

                        if (!id) {
                          console.error("❌ Missing ID, aborting delete", s);
                          return;
                        }
